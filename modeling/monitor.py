...
class ForecastMonitor(object):
    
    def __init__(self, model, report_filename):
        self.model = model
    ...
    def __load_report(self):
        ...
        """
        # Load dataset
        if not os.path.isfile(self.report_filename):
            raise IOError, "Please specify a correct file with forecast monitoring report. File not found: " \
                + self.report_filename
        self.report = pd.read_csv(self.report_filename)

    def __build_data(self):
        """
        
        """
        self.report = self.report.dropna(axis=0)
        self.report['External Id'] = self.report['External Id'].astype('int64')
        # Save only promotions in status 110 or above that do not exist in training dataset
        self.report = self.report[~self.report['External Id'].isin(self.model.data['promotion_ext_id'])]
        self.report = self.report[self.report['Promotion Status'].isin(['110 - In Settlement','130 - Finished - Evaluated'])]
        # Join our dataset to the report
df2 = df_rep.merge(df, how='left', on=['account_banner','promotion_ext_id','prod_desc'])
# df2 = df2[(df2['account_banner']=='Lenta')&(df2['promotion_status']==110)]
df2.shape
df_X = df2[remaining_feat]
pred = model.predict(df_X)
pred.columns = ['predicted']
pred.shape
df3 = pd.concat([df2, pred],axis=1)[['account_banner','promotion_ext_id','promotion_ext_id','prod_desc',
                                     'baseline_units','baseline_units_rep',
                                     'total_units','predicted','predicted_rep']]
df3['predicted'] = np.round(df3['predicted'])
df3['predicted_rep'] = np.round(df3['predicted_rep'])
df3.shape
# Absolute persentage error
df3['predicted_diff'] = np.round(100 * np.abs((df3['predicted'] - df3['predicted_rep'])/df3['predicted']))
